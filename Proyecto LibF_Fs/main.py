...
def mostrarCompras():
    cur = mysql.connection.cursor()
    cur.execute('SELECT fecha_Compra, libro_Comprado, libro.precio_Libro FROM compra JOIN libro ON libro_Comprado = libro.id_Libro WHERE compra.usuario_Comprador = "fedeBruschi"')
    compras = cur.fetchall()
    return render_template('muestraCompras.html', compras = compras)
...
def comprarLibro():
    cur = mysql.connection.cursor()
    cur.execute('SELECT id_Libro, nombre_Libro FROM libro WHERE stock_Libro > 0')
    librosCom = cur.fetchall()

    if request.method == 'POST':
        cur = mysql.connection.cursor()
        nombre_lib = request.form['libros_Compra']
        cur.execute('UPDATE libro SET stock_Libro = stock_Libro - 1 WHERE id_Libro = %s', (nombre_lib))
        mysql.connection.commit()
